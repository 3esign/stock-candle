...
- 2026-09-13: First gh-pages push was safely rejected as non-fast-forward. Cause: a local clone inherited the source local gh-pages branch, which lagged its GitHub remote; the API preflight alone did not refresh git objects. Remedy: fetch the exact public ref, verify its hash against the preflight, create a new branch from it and apply only the five reviewed public files. Never force-push.
